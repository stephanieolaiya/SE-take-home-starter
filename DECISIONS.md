...
**Assumed downstream consumer:** this feeds the same internal Pathos analyst platform the rest of this codebase serves (trial dashboards, risk scoring, dose/response analysis) — not a public data release. That assumption drives several of the calls below, especially around date precision and PII handling.
...
**Why:** a naive parser (e.g. always assuming `MM/DD/YYYY`) would silently produce a wrong date for `PT-028` while looking successful. The "only one side is a valid month" rule is deterministic and auditable. When elimination doesn't produce a unique answer, guessing would fabricate data so quarantining is the honest response.

**Alternatives considered:** always assume one fixed convention (e.g. `MM/DD/YYYY`) — rejected, this dataset proves that assumption wrong for at least one row. Ask a human for every non-ISO date — rejected as unnecessarily conservative; the disambiguation rule resolves the vast majority of cases correctly and auditably.
...
**Decision:** quarantine. Range: `age` must be `≥ 0` and `≤ 120`; `weight_kg` must be `> 0` and `≤ 300`. These deliberately treat zero differently: `weight_kg = 0` is a physical impossibility for a living enrolled patient, but `age = 0` is a legitimate, real value (a patient under one year old is conventionally recorded as age `0`) — nothing in this codebase (`data.ts`, `ARCHITECTURE_PROMPT.md`, or the README) restricts these trials to adult-only populations, so `age = 0` isn't implausible here, just unusual relative to this particular 51-row sample's *observed* range (44–77). Observed range in a sample isn't the same as an eligibility criterion, and quarantining on the former would be inventing a rule the data never actually states. (Briefly "fixed" to reject `age = 0` for consistency with `weight_kg`'s zero-exclusion, then reverted on reconsideration — the two fields aren't actually analogous, and the earlier change encoded an assumption about trial eligibility that has no source anywhere in the codebase.) Never auto-"correct" a value by guessing intent (e.g. assuming `-3` meant `3`, or `30`, or `43`) — there's no way to know, and fabricating a fix is worse than flagging it.

**Why:** this is the same principle as missing-required-fields: guessing invents data. A range check with a documented threshold is defensible and auditable; a "smart" correction isn't.
...
**Note on `dose_level=600mg BID` (`PT-037`):** extreme relative to the rest of the dataset, but *not* quarantined — the row is a Phase I dose-escalation cohort and the note explicitly documents a DLT (dose-limiting toxicity) event. The abnormal value here is the clinically meaningful signal the row exists to capture, not a data error. `dose_level` is accepted as free text generally (see below) rather than range-checked, since valid values vary by trial/drug (`400mg BID` vs. `vemurafenib + cobimetinib`).
...
**Alternatives considered:** quarantine the whole row instead of redacting — a more conservative option, genuinely defensible, but loses real clinical content for a compliance concern that redaction already resolves. Skip PII scanning of free text entirely — rejected outright; free-text PII leakage is a real, demonstrated compliance risk (this file contains an actual SSN reference and a patient name+MRN combo), not a hypothetical.
...
**Alternatives considered:** keep the first-seen row / keep the last-seen row by file order — rejected, row order in a CSV is an accident of how the file was assembled, not a meaningful signal. Quarantine both versions for manual review instead of auto-resolving — a more conservative, genuinely defensible option; not chosen because the evidence for recency here is strong enough (a real later timestamp, corroborated by explicit note text) that automatic resolution seems reasonable, but this is exactly the kind of case that would flip to manual-review-only if the conflicting fields were safety-critical (e.g. differing `adverse_events` lists) rather than a response-assessment update.
...
## `dose_level` — accepted as free text
...
**Decision:** no transformation applied — the em-dashes are preserved as valid UTF-8 rather than stripped, since this app is Unicode-aware end-to-end (Node/Express/JSON) and there's no ASCII-only downstream consumer requiring transliteration.

**Why documenting a negative finding matters:** the assignment lists this as a category the data *may* contain ("including but not limited to"), not a guarantee every category has an instance. Confirming absence (with the verification method shown) is a more honest and useful answer than skipping the category or fabricating a finding to check a box.

**Defensive check added anyway, for future data drops.** `runDataPipelineFromFile()` originally read the file with `readFileSync(path, "utf-8")`, which decodes *leniently* — Node silently substitutes `�` for invalid byte sequences instead of erroring. That means a future CRO export in a different encoding (Windows-1252 is common from Excel-on-Windows, especially once a file has em-dashes or smart quotes typed into it) would silently corrupt `lab_notes` and other free-text fields with zero indication anything went wrong — no error, no quarantine flag, just quietly wrong data in what looks like a normal clean row. Switched to `TextDecoder("utf-8", { fatal: true })`, which throws instead of substituting, so a wrong-encoding file now fails loudly at the file-read boundary rather than degrading silently downstream.

**File schema check added too.** `parseCsv()` now verifies the header contains every column `validateAndTransformRow` actually reads, throwing immediately if any are missing. Without this, pointing the pipeline at the wrong file (or a CRO export with a changed schema) wouldn't produce a clear error — every row would just individually fail with a wall of confusing `missing_patient_id`/`missing_trial_id`/etc. reasons, since none of the expected columns would exist. That looks like "51 rows are all broken" when the real diagnosis is "this isn't the expected file at all" — same principle as the field-count-mismatch check for the unquoted-comma case above: fail once, clearly, at the structural level, rather than let a structural problem cascade into misleading per-row noise.
...
A stricter formal de-identification standard was raised as a reference point mid-build. Evaluated point by point against what this assignment's existing types (`PatientRecord`, `PipelineResult` in `src/types.ts`) and stated use case (internal Pathos analyst platform, not public data release) actually call for:
