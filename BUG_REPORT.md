...
- Fix nothing here and instead fix the symptom (writeHead-before-streamText / swallowed error). That's a separate bug worth fixing on its own: it means *any* mid-stream failure (not just a missing key) currently returns a fake `200`, but it wouldn't have addressed the root cause of the key never loading. Flagged here as a follow-up and fixed separately as Bug 11.
...
**Why this fix is correct and minimal:** A root-level `.gitignore` is the standard, minimal mechanism for this: it requires no process changes, no new dependencies, and closes the gap for every future edit to `.env`. 

**Note on testing:** Like Bug 1, this isn't application logic, so there's no `vitest` unit test for it. The equivalent regression check is a CI step (e.g., `git check-ignore starter/.env` exits `0`, or `git ls-files | grep -q '^starter/\.env$'` exits nonzero) that fails the build if `.env` is ever tracked.
...
This also removes the `as any` cast (which is bad typescript practice). `parsed.data.focus` is properly typed as `AnalysisFocus` by `zod` inference, so the type system is actually enforced end-to-end now, not just declared in `types.ts` and ignored at the boundary.

**Why this fix is correct and minimal:** `zod` was already a dependency, so this adds zero new packages. Validating at the route boundary (before any OpenAI call is made) means invalid requests fail fast with a clear `400` and never reach `streamAnalysis`, so no wasted API cost and no more `writeHead(200, ...)` being committed for a request that was never going to produce a real result.
...
**Tests added:** `src/__tests__/trials.test.ts`: the existing (previously failing) `sorts by startDate` test now passes as-is, kept under its original name; added `sorts by startDate ascending when order=asc (oldest first)` alongside it. Testing both directions explicitly (rather than only the default) ensures a future regression that re-inverts either direction gets caught, not just one. Confirmed both pass via `npm test` (11/11 passing overall, no regressions in the other sort fields).

---

## Bug 5: `sort`/`order` accept any value silently instead of rejecting unsupported ones

**How I found it:** Code read and noticed that response rate was not a sort option but `curl "http://localhost:3000/trials?sort=responseRate"` returned `200` with all 8 trials, but not sorted by `responseRate` at all (neither ascending nor descending `responseRate)`.

**Root cause:** `src/services/trial-service.ts:70-83`. There is no validation for accepted sort and order values. The `switch (sortField)` only has cases for `"enrollment"`, `"startDate"`, and `"adverseEventRate"` (the three fields actually implemented). Any other value for `sort` (or, separately, any value for `order` other than exactly `"asc"`) falls through to `default: cmp = 0`, and since `Array.prototype.sort` is stable, a constant `cmp` of `0` just preserves the original array order. Nothing validates `sort`/`order` anywhere, so the client gets a `200` and a full trial list back: it looks like a valid response that simply wasn't sorted by what was asked for.

**The endpoint accepts unsupported** `sort`**/**`order` **values and silently no-ops instead of telling the caller their request wasn't honored.**
...
**Why this fix is correct and minimal:** It validates exactly against what's already implemented with no behavior change for any request that was already correctly handled. Only note is the comment on the schema to stop this list from silently drifting out of sync with the switch statement in `trial-service.ts` in the future.

**Alternatives considered:**

- Implement `responseRate` (and/or other fields) as newly-sortable to fix the gap. Rejected after reconsidering scope as this is a feature addition that does not address the actual lack of validation bug fix.
...
**How I found it:** `curl -i http://localhost:3000/trials/NCT-003/summary` returned `500` with an unhandled `TypeError: Cannot read properties of null (reading 'toFixed')`, thrown from `src/services/analysis-service.ts:84`. `NCT-003` is one of seed trial that has  `responseRate: null`. 
...
**Real-world impact:** Any analyst viewing a summary for an in-progress trial with no results yet which is a completely normal state for trial as seen in the example, gets a `500` instead of a summary, with no way to know it's a null-handling bug versus a real outage. 
...
**Why this fix is correct and minimal:** One line, no behavior change for trials that already have a response rate, and it reuses the exact convention (`"not yet available"`) already established elsewhere in the same file so no new pattern introduced.
...
**How I found it:** Read the code and spotted the incorrect use of .includes for array text search. `src/services/trial-service.ts:52-63`, inside the `search` filter's scoring block:

```js
if (t.keyFindings.includes(query)) score += 2;   // bug
```

Verified bug with testing: `NCT-001`'s `keyFindings` array contains the sentence `"Interim analysis at 6 months shows 38.5% PSA50 response rate"`. `listTrials({ search: "psa50" })` should match on that text, but returned zero results, while `listTrials({ search: "melanoma" })`, matching against `indication` (a string field), correctly returned `NCT-002`. Confirmed directly:
...
`keyFindings` is `string[]`, so the fourth line's `.includes()` resolves to `Array.prototype.includes` instead, which checks for an *exact element match*, not substring containment inside any element of the array. It would require one entire `keyFindings` string to equal the lowercased search term verbatim.

**Real-world impact:** The "search key findings" capability is effectively dead and not functional (would result in no results in UI even when substring in key finding is matching). It contributes to the relevance score in name only and never actually fires. An analyst searching for a term that appears only in a trial's findings text (and not its name, indication, or primary endpoint) gets zero results, with nothing indicating the search silently failed to check that field.
...
- Remove the `keyFindings` clause from search entirely, since it never worked anyway. Rejected, that would delete real functionality (key findings are a legitimate place to search) to avoid a one-line fix.
...
**Root cause:** `src/services/trial-service.ts:61` (before this fix), inside the `search` filter's scoring block: `(t as any)._score = score;`. The `as any` cast (which is not best practice) bypasses TypeScript entirely to write an undeclared property directly onto a shared, cached domain object.
...
- Track scores in a request-local `Map<string, number>` (or an array of `{ trial, score }` pairs) instead of writing onto `t`, so that even a *future* re-introduction of relevance data couldn't repeat this exact mistake. This is the more defensive, but was not applied, for two reasons: (1) nothing in the codebase currently needs the score past the `score > 0` filter check on the same line; there's no relevance-sort feature today to build the extra plumbing for, so there is no need to add it; (2) I checked, and this is the *only* place in `src/services/` or `src/routes/` that mutates a trial object or the results array at all (`grep` for `.push(`, `.splice(`, bracket/dot assignment, `Object.assign`, `delete` turned up nothing else), so the shared-mutable-object risk isn't a pervasive pattern here needing a structural fix, just this one now-deleted line. If a future change reintroduces per-trial scoring, the `Map`-based approach is the right call at that point.
...
## Bug 11: `streamAnalysis` commits a `200` before it's known whether the AI call will succeed, so any failure (not just a missing key) still returns a fake success

**How I found it:** Originally assumed that Bug 1 returned 200 because the auth failure was being "swallowed" because headers were already `200` by the time it happened. That's true, but investigating this bug surfaced a second, more fundamental problem underneath it: `streamText`'s `textStream` never throws for provider or auth errors *at all*, by design ("errors become part of the stream and are not thrown to prevent servers from crashing," per the AI SDK's own troubleshooting docs). The only way to observe them is an `onError` callback. Without one, an invalid key, a rate limit, or a genuine mid-stream network failure all complete silently with zero chunks and no exception, regardless of when `writeHead` fires.

**Root cause:** `src/services/analysis-service.ts`, in `streamAnalysis`. Two compounding issues, only the first of which Bug 1 addressed indirectly: (1) `response.writeHead(200, ...)` used to run before `streamText()` was even called, so headers were committed before there was any chance to know if the call would succeed. (2) Even after fixing the timing, `streamText()` was never passed an `onError` callback, so `textStream`'s `for await` loop simply completes when the underlying request fails, no exception, no signal, nothing for a `try`/`catch` around it to ever catch.
...
+ if (streamError) {
+   const message = streamError instanceof Error ? streamError.message : "Analysis failed";
+   if (!headersSent) {
...
A failure before any chunk streams now throws, `headersSent` is correctly still `false`, and the route's existing catch block (`if (!res.headersSent) res.status(500)...`, already correct, it was just never reached) sends a real `500`. A failure after some chunks already streamed can't change the committed `200`, that's a real constraint of HTTP, but the client now gets an explicit `data: {"error": ...}` event instead of a silently truncated stream.

Verified directly: an invalid key now throws `"Incorrect API key provided..."` with `writeHead` never called and zero chunks written; a real, valid key still streams normally end to end.

**Why this fix is correct and minimal:** No new dependency, no change to the route's catch block, which was already written correctly and just needed `headersSent` to reflect reality. The `onError` callback is the AI SDK's own documented mechanism for this, not a workaround.
